...
/**
 * Al cargar la página, si hay resultado, hace scroll suave hasta él.
 */
...
function switchPromptMode(mode) {
  ...
  modes.forEach(m => {
    const panel = document.getElementById(`prompt-mode-${m}`);
    if (panel) {
...
function wizardBuild() {
  ...
  if (hints.length) {
    parts.push(`destacar: ${hints.join(', ')}`);
  }
  ...
  // Extra opcional
  const extraInput = document.getElementById('wizard-extra');
...
function selectLLMOption(option) {
  ...
  const selectedOption = document.getElementById(`llm-opt-${option}`);
  if (selectedOption) {
    selectedOption.classList.add('llm-modal__option--selected');
  }
  ...
  // Si es predeterminado, actualizar valor y cerrar
  if (option === 'default') {
    const defaultLabel =
      window.WB_I18N && window.WB_I18N.llmDefaultLabel
        ? window.WB_I18N.llmDefaultLabel
        : 'Predeterminado';
